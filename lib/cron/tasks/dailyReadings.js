...
async function dailyReadings(errors) {
  ...
  if (oilKey) {
    try {
      ...
      const [bOP, wOP, gOP] = await Promise.allSettled([
        ...
        fetch(`https://api.oilpriceapi.com/v1/prices/latest?by_code=NATURAL_GAS_USD`, { headers: oilHeaders, signal: AbortSignal.timeout(8000) }),
      ]);
      ...
      if (gOP.status === "fulfilled" && gOP.value.ok) {
        const d = await gOP.value.json();
        if (d?.data?.price > 0) { reading.natgas = parseFloat(d.data.price.toFixed(3)); }
      }
    } catch {}
  }
  ...
  // EIA fallback for natgas (Henry Hub — separate series)
  if (!reading.natgas && process.env.EIA_API_KEY) {
    try {
      const gasUrl = `https://api.eia.gov/v2/natural-gas/pri/fut/data/?api_key=${process.env.EIA_API_KEY}&frequency=daily&data[0]=value&facets[series][]=RNGWHHD&sort[0][column]=period&sort[0][direction]=desc&length=1`;
      const gRes = await fetch(gasUrl, { signal: AbortSignal.timeout(8000) });
      if (gRes.ok) {
        const gData = await gRes.json();
        const gItem = gData?.response?.data?.[0];
        if (gItem?.value) reading.natgas = parseFloat(parseFloat(gItem.value).toFixed(3));
      }
    } catch {}
  }
