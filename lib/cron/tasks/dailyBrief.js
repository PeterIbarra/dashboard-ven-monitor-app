...
═══ INSTRUCCIONES ═══
Produce un JSON con esta estructura exacta. SOLO JSON, sin backticks, sin markdown, sin texto adicional:
{
  "resumen": "1 párrafo (80-120 palabras) — valoración general del día en relación al contexto venezolano",
  "mercados": "1 párrafo (60-80 palabras) — análisis de dólar, petróleo, impacto fiscal",
  "politico": "1 párrafo (60-80 palabras) — dinámica política interna basada en las noticias",
  "internacional": "1 párrafo (60-80 palabras) — geopolítica y relación bilateral",
  "social": "1 párrafo (40-60 palabras) — conflictividad y clima social si hay datos relevantes",
...
- Las alertas deben ser las 3 más relevantes, no más.`;
  ...
  let brief;
  try {
    const cleaned = aiResult.text.replace(/```json\s*/g, "").replace(/```/g, "").trim();
    brief = JSON.parse(cleaned);
  } catch (e) {
    errors.push(`Daily Brief: JSON parse failed — ${e.message}`);
