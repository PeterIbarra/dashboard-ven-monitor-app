...
- BASE PRINCIPAL DEL ANÁLISIS: "resumen" y "riesgoDelDia"/"razonRiesgo" deben construirse PRIMERO a partir de ═══ ALERTAS CLASIFICADAS ═══ y ═══ HEADLINES RSS ═══ (los hechos concretos y verificables del día) — NO a partir de los paneles cuantitativos del dashboard (Mercados, ICG, GDELT, IODA, ACLED, etc.), que son solo contexto de apoyo para explicar POR QUÉ una noticia importa, nunca el punto de partida.
  · Si la alerta prioritaria #1 (la que vas a poner primero en "alertas") describe un hecho concreto (ej. "apagón afecta a cinco estados"), "resumen" tiene que nombrar ESE hecho tal cual, con sus detalles concretos — NUNCA disolverlo en una categoría genérica tipo "crisis energética persistente" o "tensiones internas". Lo mismo para las demás alertas que uses.
  · "razonRiesgo" debe citar textualmente cuál(es) de las alertas/noticias concretas justifican el nivel de riesgo — no una frase genérica de categoría (evita "tensiones internas, crisis energética y dependencia de acuerdos internacionales"; en su lugar algo como "el apagón que dejó a cinco estados sin luz y la escalada verbal Cabello-Padrino son los factores que más pesan hoy").
  · Los datos cuantitativos (Mercados, ICG, GDELT, Bilateral, IODA, ACLED) se usan SOLO para enriquecer o cuantificar una noticia ya mencionada (ej. "el dólar paralelo cerró en X Bs, ampliando la brecha") — no para introducir un tema que ninguna alerta o titular respalda.
  · No repitas los mismos 2-3 titulares de siempre si las alertas/headlines de HOY traen algo distinto — sigue lo que digan las fuentes de HOY, no un patrón fijo de temas.`;
