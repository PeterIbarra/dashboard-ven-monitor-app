// ═══════════════════════════════════════════════════════════════
// DAILY BRIEF — AI-generated situational email via SendGrid
...
async function sendDailyBrief(errors) {
  ...
  if (!SENDGRID_KEY) { errors.push("SENDGRID_API_KEY not configured"); return { sent: false, reason: "no_key" }; }
...
<div style="max-width:640px;margin:0 auto;background:#0d1117;color:#e6edf3;">
  ...
  // ═══ 5. SEND VIA SENDGRID ═══
  const fromEmail = process.env.DAILY_BRIEF_FROM || "";
  ...
  if (!fromEmail) {
    errors.push("DAILY_BRIEF_FROM not configured (sender email)");
  ...
  if (sgRes.status !== 202 && sgRes.status !== 200) {
    ...
    errors.push(`SendGrid: ${sgRes.status} — ${sgBody}`);
    return { sent: false, reason: "sendgrid_error", status: sgRes.status, detail: sgBody };
  }
  ...
  return {
    ...
    provider: aiResult.provider,
    emailId: sgRes.headers?.get("x-message-id") || null,
  };
}
