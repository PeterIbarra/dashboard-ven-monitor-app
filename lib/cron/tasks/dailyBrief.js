...
async function sendDailyBrief(errors) {
  ...
  const [readings, rates, alertRows, headlines, icgRows, prevBriefs, conectividad, conflictividad, sismos, ambiental] = await Promise.allSettled([
    ...
    sbQuery("daily_briefs", "select=date,risk_level&order=date.desc&limit=2"),
    getConectividad(),
    getConflictividad(),
    getSismos(),
    getAmbiental(),
  ...
  const conflictividadText = conf.ok
    ? `Eventos ACLED (últimos 7 días): ${conf.count}${conf.daysSinceLastEvent != null ? ` — dato más reciente cargado hace ${conf.daysSinceLastEvent} días (ACLED actualiza semanalmente, no es necesariamente de hoy)` : ""}` +
  ...
  const ambientalText = ambient.ok
    ? `Focos de calor VIIRS (24h, territorio nacional): ${ambient.count}${ambient.significant ? " — ACTIVIDAD SIGNIFICATIVA" : ""}`
...
<div style="max-width:640px;margin:0 auto;background:#0d1117;color:#e6edf3;">
  ...
  <!-- CONFLICTIVIDAD (ACLED — actualización semanal, no diaria) -->
  ...
  <div style="padding:16px 24px;background:#161b22;border-bottom:1px solid #21262d;">
    ...
    <div style="font-size:12px;color:#8b949e;margin-bottom:6px;">Eventos registrados: <span style="color:#e6edf3;font-weight:600;">${conf.count}</span>${conf.daysSinceLastEvent != null ? ` <span style="color:#484f58;">· dato más reciente: hace ${conf.daysSinceLastEvent}d (carga semanal)</span>` : ""}</div>
  ...
  <!-- AMBIENTAL (solo si hay actividad significativa de incendios) -->
  ${ambient.ok && ambient.significant ? `
  <div style="padding:16px 24px;background:#161b22;border-bottom:1px solid #21262d;">
    <div style="font-size:10px;letter-spacing:0.15em;text-transform:uppercase;color:#f97316;margin-bottom:8px;">Ambiental — Incendios (24h, NASA FIRMS)</div>
    <div style="font-size:12px;color:#8b949e;">Focos de calor detectados: <span style="color:#e6edf3;font-weight:600;">${ambient.count}</span></div>
