...
async function callAICascade(prompt, maxTokens = 800, temperature = 0.3) {
    ...
    try {
      ...
      if (prov.name === "gemini") {
        ...
        const r = await fetch(prov.url, {
          method: "POST", headers,
          body: JSON.stringify({ model: prov.model, messages: [{ role: "user", content: prompt }], max_tokens: maxTokens, temperature }),
          signal: AbortSignal.timeout(25000),
        });
