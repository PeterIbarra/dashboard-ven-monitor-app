...
export function TabOpinionPublica({ section, setSection }) {
  ...
  return <div>
    ...
    {section==="panorama" && <>
      <div style={{ display:"grid", gridTemplateColumns:mob?"repeat(2,1fr)":"repeat(4,1fr)", gap:10, marginBottom:12 }}>{OPINION_SNAPSHOT.cards.map(c=><Card key={c.label} accent={c.color}><div style={{ fontSize:9, fontFamily:font, color:MUTED, textTransform:"uppercase", minHeight:28 }}>{c.label}</div><div style={{ fontSize:mob?24:30, fontWeight:800, color:c.color }}>{c.value}{c.suffix}</div><div style={{ fontSize:10, color:MUTED }}>{c.delta}</div></Card>)}</div>
      ...
      <Card accent="#dc2626" style={{ marginTop:12, background:"linear-gradient(135deg,#fff 55%,#fef2f2)" }}>
        <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1.1fr 1fr", gap:18, alignItems:"center" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8 }}>{[{label:"Desconfianza oficial",value:94.1,color:"#dc2626"},{label:"Respuesta negativa",value:91.6,color:"#dc2626"},{label:"Solidaridad",value:90.3,color:"#2d8a30"},{label:"Crisis habitacional",value:64,color:"#ca8a04"}].map(x=><div key={x.label} style={{ border:`1px solid ${BORDER}`, background:"#fff", padding:"9px" }}><div style={{ fontSize:18, fontWeight:800, color:x.color }}>{x.value}%</div><div style={{ fontSize:9, color:MUTED }}>{x.label}</div></div>)}</div>
        </div>
      </Card>
