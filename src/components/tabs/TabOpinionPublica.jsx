...
export function TabOpinionPublica({ section, setSection }) {
  ...
  return <div>
    ...
    {section==="panorama" && <>
      ...
      <div style={{ background:"#fff7ed", border:"1px solid #fdba7444", padding:"11px 14px", fontSize:11, color:"#9a3412", lineHeight:1.55 }}>Lectura: AtlasIntel documenta desaprobación del Gobierno y división sobre el acuerdo petrolero; Meganalisis registra desconfianza en el diálogo e intención de voto. Las dos encuestas tienen universos, modos y preguntas diferentes: no se promedian ni se leen como una misma serie. Las mediciones anteriores siguen disponibles en el archivo.</div>
    </>}

    {section==="liderazgo" && <Card><div style={{ fontSize:11, fontFamily:font, color:MUTED, textTransform:"uppercase", marginBottom:14 }}>Imagen y aprobación de figuras · indicadores publicados con ambos polos</div><div style={{ display:"grid", gap:12 }}>{LEADERSHIP.map(p=><div key={p.name} style={{ display:"grid", gridTemplateColumns:mob?"1fr":"190px 1fr 1fr 70px", gap:mob?5:12, alignItems:"center", borderBottom:`1px solid ${BORDER}`, paddingBottom:10 }}><b style={{ fontSize:12 }}>{p.name}</b><div><div style={{ display:"flex", justifyContent:"space-between", fontSize:9, color:MUTED }}><span>Positiva</span><span>{p.positive}%</span></div><Bar value={p.positive} color="#2d8a30"/></div><div><div style={{ display:"flex", justifyContent:"space-between", fontSize:9, color:MUTED }}><span>Negativa</span><span>{p.negative}%</span></div><Bar value={p.negative} color="#dc2626"/></div><div style={{ fontSize:12, fontWeight:800, color:p.balance>=0?"#2d8a30":"#dc2626", textAlign:mob?"left":"right" }}>{p.balance>0?"+":""}{p.balance} pp</div></div>)}</div><div style={{ fontSize:10, color:MUTED, marginTop:12 }}>Fuente: AtlasIntel/Bloomberg LATAM Pulse, corte citado en el informe 4–11 sep. Machado: imagen positiva/negativa; Trump: aprobación/desaprobación. El saldo se calcula dentro de cada pregunta, sin comparar los dos saldos como una misma métrica.</div></Card>}
