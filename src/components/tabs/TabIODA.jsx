...
export function TabIODA() {
  ...
  const loadRegions = useCallback(async () => {
    ...
    try {
      // C4: independent confirmation using already-loaded events (eventsRef) —
      // preserved from the original inline implementation so the tab keeps
      // its exact behavior (unlike the background loader, which omits it).
      const cachedEvts = eventsRef.current;
      const iodaConfirmsState = (stateName, t1, t2) => cachedEvts.some(e =>
        e.region === stateName && e.datasource === "ping-slash24" &&
        e.time >= t1 - 3600 && e.time <= t2 + 3600
      );
      const scores = await computeRegionElectric({ twFrom, twUntil, iodaConfirmsState });
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:0, border:`1px solid ${BORDER}`, marginBottom:14 }}>
        {[{id:"estados",label:"🗺 Estados"},{id:"eventos",label:"⚡ Eventos"},{id:"electricidad",label:"🔌 Cortes eléctricos"},{id:"nacional",label:"📡 Nacional"}].map(s => (
      ...
      {subView === "electricidad" && (() => {
        ...
        return (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {/* Section 2: Possible/inferred — degradation without an explicit alert-cluster */}
            <Card accent="#f97316">
              <div style={{ fontSize:13, fontFamily:font, color:MUTED, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>
